                ...
                <div class="col-1">
                    <div class="language-changer language-changer-desktop">
                        ...
                        if (is_category()) {
                            ...
                            if (isset($queried_object->slug) && ($queried_object->slug === 'news-ar' || strpos($queried_object->slug, 'news') !== false)) {
                                $is_news_page = true;
                            }
                        } elseif (is_single() && get_post_type() == 'post') {
                            $post_categories = get_the_category();
                            foreach ($post_categories as $cat) {
                                if ($cat->slug === 'news-ar' || strpos($cat->slug, 'news') !== false) {
                                    $is_news_page = true;
                                    break;
                                }
                            }
                        ...
                        if (is_category() || is_archive()) {
                            ...
                            if (isset($queried_object->slug)) {
                                ...
                                if (strpos($slug, 'companies-services-ar') !== false) {
                            ...
                            $post_categories = get_the_category();
                            foreach ($post_categories as $cat) {
                                if (strpos($cat->slug, 'companies-services-ar') !== false) {
                                    $is_services_page = true;
                                    $service_type = 'companies';
                                    break;
                        ...
                        if ($is_news_page) {
                            // Switch to English news category
                            $news_category = get_category_by_slug('news');
                            if ($news_category) {
                                $english_url = get_category_link($news_category->term_id);
                            }
                        }
