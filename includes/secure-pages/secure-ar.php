...
<div class="hero">
    <div class="hero-background">
        <div class="hero-content ">
            ...
            <h2 class="hero-title"> تقدم تنوع من الحلول القانونية<br/> والاستشارات الأمنية</h2>
            <p class="hero-subtitle lead">تعلم كيفية حماية حقوقك من خلال إرشاداتنا القانونية المتخصصة.</p>
        </div>
    </div>
...
<div class="wrapper">
            ...
            if (is_category()) {
                ...
                // Get secure-yourself parent category using Polylang
                $secure_en = get_category_by_slug('secure-yourself');
                if (!$secure_en) {
                    $secure_en = get_term_by('name', 'Secure Yourself', 'category');
                }
                
                if ($secure_en && function_exists('pll_get_term')) {
                    $secure_ar_id = pll_get_term($secure_en->term_id, 'ar');
                    if ($secure_ar_id) {
                        $display_category = get_category($secure_ar_id);
                    }
                }
                
                // Fallback
                if (!$display_category) {
                    $display_category = $secure_en;
                }
            }
            ...
            if ($display_category && !empty($display_category->description)) {
                ...
                echo '<p class="subtitle lead">في ظل المشهد القانوني المعقد اليوم، أصبح فهم كيفية حماية حقوقك أكثر أهمية من أي وقت مضى. في شركة داغ للمحاماة والاستشارات، يكرس فريقنا من المحامين ذوي الخبرة جهوده لتزويدك بالمعرفة والأدوات التي تحتاجها لتأمين نفسك قانونياً.</p>';
            }
            ?>
        </div>
    <div class="container overflow-hidden secure-grid-container">
 ...
 // Only show navigation for category archives with subcategories
    ...
    // Get secure-yourself category using Polylang
    $secure_category = null;
    if (function_exists('pll_get_term')) {
        ...
        if ($secure_en) {
            if ($current_lang === 'ar') {
                $secure_ar_id = pll_get_term($secure_en->term_id, 'ar');
                if ($secure_ar_id) {
                    $secure_category = get_category($secure_ar_id);
                }
            } else {
                $secure_category = $secure_en;
            }
        }
    }
    ...
    if (!$secure_category) {
        if ($current_lang === 'ar') {
            $secure_category = get_category_by_slug('secure-yourself-ar');
            if (!$secure_category) {
                $secure_category = get_term_by('name', 'احم نفسك', 'category');
            }
        } else {
